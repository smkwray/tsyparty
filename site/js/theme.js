/* Theme system — runs in <head> before DOM renders to prevent FOUC.
 * Follows xchratlas pattern: localStorage > system preference > default.
 * Declares color-scheme so browsers don't force-invert elements. */
(function () {
  'use strict';
  var KEY = 'tsyparty-theme';
  ...
  /* Apply immediately — this runs in <head> before body renders */
  var initial = saved() || sys();
  apply(initial);

  /* Re-apply button icon once DOM is ready (button doesn't exist yet in <head>) */
  ...
  /* If no saved preference, follow system preference changes in real-time */
